...
Учебный/портфолио-проект: полнофункциональный интернет-магазин на .NET 8 + React. Публичная часть (каталог, корзина, оформление заказа) и админ-панель (заказы, товары, категории, пользователи) в одном репозитории.

Полное техническое задание — [TECH_SPEC.md](TECH_SPEC.md). Сводка проекта для нового участника (или нового диалога с Claude) — [docs/context/](docs/context/README.md). Пошаговый статус разработки и обоснование ключевых решений — в [docs/PROGRESS.md](docs/PROGRESS.md) и [docs/DECISIONS.md](docs/DECISIONS.md) (там же — детали, которые не поместились сюда: почему выбраны те или иные версии пакетов, найденные и исправленные по ходу разработки нестыковки между ТЗ и реализацией и т.п.).
