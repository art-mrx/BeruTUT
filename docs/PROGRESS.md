...
## Доработка после MVP: управление пользователями в админке (по запросу пользователя, 2026-09-21) — `done`
Вкладка «Пользователи»: полный список, блокировка и разблокировка.
- [x] `User.IsBlocked` + `BlockedAt` (миграция `AddUserBlocking`, у существующих пользователей `false`)
- [x] `GET /api/admin/users` (Admin): `search` (имя/email/номер), `isBlocked`, `sortBy=newest|oldest`, пагинация; в ответе id (номер), email, имя, роль, дата регистрации, число заказов, статус блокировки. `POST /api/admin/users/{id}/block` и `/unblock`
- [x] **Заблокированный не может войти**: логин → 403 (только после проверки пароля — при неверном пароле по-прежнему 401, состояние аккаунта не раскрывается), обновление токена → 403, а уже выданный access-токен отклоняется **сразу** (JWT-событие `OnTokenValidated` проверяет пользователя в БД на каждом запросе), плюс при блокировке отзываются все refresh-токены
- [x] Администраторов заблокировать нельзя (409) — заодно защита от блокировки самого себя
- [x] Frontend: вкладка «Пользователи» (таблица, поиск, фильтр «Активные/Заблокированные», сортировка, «Сбросить», подтверждение блокировки); у заблокированного при входе — понятное сообщение; если заблокировали посреди сессии — выкидывает на `/login` с пояснением
- [x] Тесты 66/66 (23 новых). Проверено на настоящем PostgreSQL по HTTP: токен до блокировки 200 → после блокировки тот же токен 401, refresh 403, вход 403, неверный пароль 401, админ 409, покупатель 403, unknown 404, разблокировка → вход 200. UI проверен в браузере, включая блокировку посреди сессии и мобильную ширину
